...
public class ParallelHuffmanEncoder3 {
	AtomicIntegerArray aFreq;
	IntegerBitArray[] codes;
	ArrayList<String> inputChunks;
	
	public ParallelHuffmanEncoder3(int threadCount, String filename) {
		...
		try {
			while(fr.ready()) {
				comp = "";
				while(fr.ready() && readChars < charLimit) {
					comp += (char)fr.read();
					readChars++;
				}
				readChars=0;
				inputChunks.add(comp);
				StringReader r = new StringReader(comp);
		...
		Driver.specificResults[6][Driver.trialNum] = treeTimer.stop();
		...
		writeData(w, root);
		w.close();
		Driver.specificResults[7][Driver.trialNum] = treeTimer.stop();
	...
	private void writeData(BinaryOut w, HuffmanNode root) {
		for(String m : inputChunks) {
	...
	private class StringReader implements Runnable{
		String s;
